"""Bulk register rpow2.com accounts — curl_cffi TLS impersonation.

Strategy: CapSolver gives a valid Turnstile token. Use curl_cffi with
Chrome TLS impersonation to call /auth/request — bypasses Cloudflare TLS
fingerprinting that plain requests/httpx fail with.

Usage:
    python _bulk_reg_curl.py <target> [<parallel>]
...
def do_register_httpx(email_addr, turnstile_token):
    """Fallback: use httpx with Chrome-like TLS via HTTPX_HTTP2 and chrome certificate."""
    import httpx
...
def verify_and_get_session(verify_url):
    """Visit verify URL using curl_cffi and look for session in cookies or response."""
    from curl_cffi import requests as curl
    ...
    try:
        resp = curl.get(verify_url, headers=headers, proxy=proxy, impersonate="chrome",
...
def worker(wid):
    ...
    if not session:
        log(f"[w{wid}] no session extracted from verify URL")
        return None
