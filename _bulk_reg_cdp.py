"""Bulk register rpow2.com accounts — CDP (DevTools Protocol) for API calls.

Key insight: Cloudflare blocks fetch() calls made from INSIDE the page context
(even with a valid Turnstile token). But CDP Network.* commands run OUTSIDE
the page context and have full browser TLS/UDP fingerprints.

Usage:
    python _bulk_reg_cdp.py <target> [<parallel>]
...
import os, sys, json, time, random, base64, threading, imaplib, email, re, urllib.parse, tempfile, uuid
...
def new_driver(proxy_host, proxy_port, proxy_user, proxy_pass):
    """Fresh undetected_chromedriver with proxy auth. Each call = fresh browser."""
...
def cdp_fetch(driver, url, method="GET", headers=None, body=None):
    """Use Chrome DevTools Protocol to make an HTTP request outside the page context.
    This runs at the browser level with full TLS fingerprints, bypassing page-level
    Cloudflare restrictions on fetch()."""
    session_id = driver.session_id

    # Enable Network domain
    driver.execute_cdp_cmd("Network.enable", {})

    # Create the request ID tracking map
    request_id = [None]

    def capture_request():
        # This script runs in the browser and intercepts the request
        return """
        new Promise((resolve) => {
            const reqId = window.__cdp_req_id__;
            if (reqId && window.__cdp_response__) {
                resolve(window.__cdp_response__);
            } else {
    ...
    # Use execute_cdp to make the actual HTTP request
    # Network.performInteraction handles the network layer
    try:
        # First, navigate to a blank page so we have a clean context
        driver.get("about:blank")
        time.sleep(0.5)

        # Use the Chrome debugging protocol directly via execute_cdp
        # to perform a fetch outside the page context
        result = driver.execute_cdp_cmd("Fetch.execute", {
            "request": {
                "url": url,
                "method": method,
        ...
        # If CDP Fetch doesn't work, fall back to a simple approach
        return {"error": str(e)}


def do_register_via_cdp(driver, email_addr, turnstile_token):
    """Use CDP (outside page context) to POST /auth/request with full TLS fingerprints."""
    try:
        # Navigate to rpow2.com first to set Cloudflare cookies / challenges
        driver.get(PAGE_URL)
        time.sleep(5)  # Let Cloudflare settle

        # Now use CDP to make the API call from OUTSIDE the page JS context
        # CDP commands run at browser level, not page level
        body = json.dumps({"email": email_addr, "turnstile_token": turnstile_token})

        # Use fetch domain via CDP
        try:
            response = driver.execute_cdp_cmd("Fetch.execute", {
                "request": {
                    "url": f"{RPOW_API}/auth/request",
                    "method": "POST",
                    "headers": {
            ...
            if response and "response" in response:
                resp_body = response["response"].get("responseBody", "{}")
                try:
                    return json.loads(resp_body)
                except Exception:
                    return json.loads(resp_body if isinstance(resp_body, str) else "{}")
            return response
        except Exception as cdp_err:
            log(f"[w] CDP Fetch error: {cdp_err}")

        # Fallback: try execute_script but wait much longer for Cloudflare clearance
        driver.get(PAGE_URL)
        time.sleep(8)
        script = f"""
        return fetch("{RPOW_API}/auth/request", {{
            ...
            body: JSON.stringify({{email: "{email_addr}", turnstile_token: "{turnstile_token}"}})
...
def do_register(email_addr, turnstile_token, driver):
    """Use Chrome to call /auth/request. Prefer CDP, fallback to execute_script."""
    try:
        driver.get(PAGE_URL)
        time.sleep(6)
    except Exception:
    ...
    try:
        driver.execute_cdp_cmd("Network.enable", {})
        cdp_resp = driver.execute_cdp_cmd("Fetch.execute", {
            "request": {
                "url": f"{RPOW_API}/auth/request",
                "method": "POST",
                "headers": headers,
        ...
        if cdp_resp and "response" in cdp_resp:
            raw = cdp_resp["response"].get("responseBody", "{}")
